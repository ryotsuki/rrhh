...
if($accion=="entrar"){
	...
	$login = "SELECT * FROM usuario_pedidos WHERE correo_usuario_pedido='".$usuario."' AND clave_usuario_pedido = '".$clave."' AND id_status = 1";
	$consulta_login = sqlsrv_query($conn2,$login);	

	$numero_filas = sqlsrv_has_rows($consulta_login);
	
	if($numero_filas){
	
		while($row=sqlsrv_fetch_array($consulta_login)) {
	
			$_SESSION['user_email_address']	= $row["correo_usuario_pedido"];
            $_SESSION['user_first_name'] 	= $row["empresa_pedido"];
            $_SESSION['user_last_name']     = $row["persona_responsable_pedido"];;
			$_SESSION['id_status_usuario'] 	= $row["id_status"];
            $_SESSION['clave_usuario'] 		= $row["clave_usuario_pedido"];
            $_SESSION['ultimoacceso'] 		= date("Y-n-j H:i:s");
            $_SESSION['id_usuario'] 		= $row["id_usuario_pedido"];
			
            header("location: pedidos_eta");
...
<head>
    ...
    <title>Novomode :: Log in</title>

    <script>
        function login(){

            document.frm_login.action ="login_pedidos_eta?accion=entrar";
...
<body class="m4-cloak h-vh-100 d-flex flex-justify-center flex-align-center">
    ...
    <div class="login-box">
        ...
        >
            <br>
            <img src="images/new-balance-logo.png" class="mt-4-minus mr-6-minus">
            <div class="text-muted mb-4 place-center">Ingrese sus datos para iniciar sesión</div>
            <div class="form-group">
                <input type="text" id="txt_correo" name="txt_correo" data-role="input" placeholder="Correo" data-append="<span class='mif-envelop'>" data-validate="required">
            ...
            <div class="form-group">
                ...
                <span class="invalid_feedback">Por favor, ingrese su contraseña</span>
            </div>
