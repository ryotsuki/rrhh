...
if($almacen != ""){
    ...
    $consulta_almacen2 = "AND ALMACEN IN($new_almacen)";
}
else{ 
    $consulta_almacen = "";
    $consulta_almacen2 = "";
...
$sql = "SELECT DISTINCT
            ...
            C.E_MAIL AS CORREO,
            C.TELEFONO1 AS CELULAR,
            (SELECT MAX(SEXO) FROM VENTAS WHERE V.CEDULA_RUC = CEDULA_RUC) AS SEXO,
            (SELECT TOP(1) ALMACEN FROM VENTAS WHERE V.CEDULA_RUC = CEDULA_RUC $consulta_almacen2 ORDER BY FECHA DESC) AS ALMACEN,
            ...
            CASE
            WHEN (DATEDIFF(dd,(SELECT MAX(FECHA) FROM VENTAS  WHERE V.CEDULA_RUC = CEDULA_RUC),GETDATE()) >= 270 AND DATEDIFF(dd,(SELECT MAX(FECHA) FROM VENTAS  WHERE V.CEDULA_RUC = CEDULA_RUC),GETDATE()) <= 364) THEN 'POR INACTIVAR'
            WHEN (DATEDIFF(dd,(SELECT MAX(FECHA) FROM VENTAS  WHERE V.CEDULA_RUC = CEDULA_RUC),GETDATE()) > 364)  THEN 'INACTIVO'
            WHEN (DATEDIFF(dd,(SELECT MAX(FECHA) FROM VENTAS  WHERE V.CEDULA_RUC = CEDULA_RUC),GETDATE()) < 270)  THEN 'ACTIVO'
            WHEN (DATEDIFF(dd,(SELECT MAX(FECHA) FROM VENTAS  WHERE V.CEDULA_RUC = CEDULA_RUC),GETDATE()) = 330)  THEN 'POR INACTIVAR'
            END AS ESTADO,
            (SELECT SUM(CANTIDAD_VTA) FROM VENTAS WHERE V.CEDULA_RUC = CEDULA_RUC $consulta_almacen2 ) AS UNIDADES,
            (SELECT SUM(VTA_NETA) FROM VENTAS WHERE V.CEDULA_RUC = CEDULA_RUC $consulta_almacen2 ) AS VENTA_NETA,
            (SELECT COUNT(DISTINCT NUMERO_DOCUMENTO) FROM VENTAS WHERE V.CEDULA_RUC = CEDULA_RUC $consulta_almacen2 ) AS FRECUENCIA,
            (SELECT (SUM(VTA_NETA)/NULLIF(SUM(CANTIDAD_VTA),0))*(SUM(CANTIDAD_VTA)/NULLIF(COUNT(DISTINCT NUMERO_DOCUMENTO),0))  FROM VENTAS WHERE V.CEDULA_RUC = CEDULA_RUC $consulta_almacen2 ) AS VPT,
            (SELECT (SUM(CANTIDAD_VTA)/NULLIF(COUNT(DISTINCT NUMERO_DOCUMENTO),0)) FROM VENTAS WHERE V.CEDULA_RUC = CEDULA_RUC $consulta_almacen2 ) AS UPT,
            CASE 
            ...
            WHEN V.ALMACEN LIKE '%NEW %' THEN '33'
            WHEN T.IDTIPOTARJETA IS NULL THEN '44'
            END AS SEGMENTO
            FROM 
            VENTAS V LEFT JOIN
            ...
            $consulta_descuento
            GROUP BY 
            CEDULA_RUC, C.FECHANACIMIENTO, T.IDTIPOTARJETA, C.CODCLIENTE, V.ALMACEN, C.E_MAIL, C.TELEFONO1";
...
>
    ...
    <tr>
        ...
        <th class="sortable-column">VENTA</th>
    ...
    <tr>
        ...
        <td><?php echo number_format($row["VPT"],2,',','.')?></td>
        <td><?php echo number_format($row["UPT"],2,',','.')?></td>
    </tr>
    <?php
        }}
    ?>
