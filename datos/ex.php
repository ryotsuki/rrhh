...
session_start(); 
    ...
    $sql2 = "SELECT distincT CEDULA_CLIENTE, CASE   
                WHEN SEGMENTO IS NULL  THEN (SELECT MAX(SEGMENTO) FROM HISTORICO_SEGMENTOS WHERE COD_CLIENTE = A.COD_CLIENTE)
                ELSE SEGMENTO
            END AS SEGMENTO, 
            (SELECT MAX(EMAIL_CLIENTE) FROM VENTAS_MERCADEO_Y_MODA WHERE a.CEDULA_CLIENTE = CEDULA_CLIENTE) AS EMAIL_CLIENTE,
            (SELECT MAX(CELULAR_CLIENTE) FROM VENTAS_MERCADEO_Y_MODA WHERE a.CEDULA_CLIENTE = CEDULA_CLIENTE) AS CELULAR_CLIENTE,
            (SELECT MAX(NOMBRE_ALMACEN) FROM VENTAS_MERCADEO_Y_MODA WHERE a.CEDULA_CLIENTE = CEDULA_CLIENTE) AS ALMACEN,
            FECHA_CREACION,
            (SELECT SUM(VENTA_NETA) FROM VENTAS_MERCADEO_Y_MODA WHERE a.CEDULA_CLIENTE = CEDULA_CLIENTE AND  FECHA_FACTURA BETWEEN '$desdenew' AND '$hasta4') AS VENTA,
            (select COUNT(distincT (NUMERO_FACTURA))
            from [dbo].[VENTAS_MERCADEO_Y_MODA]
            where [FECHA_FACTURA] between '$desde2' and '$hasta2'
            and CEDULA_CLIENTE = a.CEDULA_CLIENTE
...
<table class="table compact striped table-border mt-4" data-role="table"
        ...
        >
            ...
            <tr>
                ...
                <th class="sortable-column sort-asc">CREACION</th>
                <th class="sortable-column sort-asc">VENTA</th>
