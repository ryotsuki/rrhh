<?PHP
    ...
    if($pedido > 0){
        ...
        $consulta1 = "UPDATE CABECERA_PEDIDO_ETA SET valor_pedido = $valor_pedido, cantidad_pares = $totales_cantidad, cantidad_referencias = $referencias_cantidad, estado_pedido = 1 WHERE id_pedido = $pedido";
        $actualiza_cabecera = sqlsrv_query($conn,$consulta1);

        $borrar = "DELETE FROM PEDIDO_ETA WHERE id_pedido = $pedido";
        ...
        foreach ($detalle_doc as $item) {
            ...
            $consulta2 = "INSERT INTO PEDIDO_ETA(id_pedido,referencia,precio,a,b,cc,em,t,w,g,p,i) 
            VALUES($pedido,'$referencia',$precio,$a,$b,$cc,$em,$t,$w,$g,$p,$i)";
            $inserta_detalle = sqlsrv_query($conn,$consulta2);
        }

        $busca_faltantes = "SELECT * FROM datos_articulos_nb_ETA WHERE style NOT IN(SELECT DISTINCT referencia FROM pedido WHERE id_pedido = $pedido)";
        $res_faltantes = sqlsrv_query($conn,$busca_faltantes);
        while($row=sqlsrv_fetch_array($res_faltantes)) {
            $refe = $row["style"];
            $prec = $row["precio_cadena"];
            $consulta3 = "INSERT INTO PEDIDO_ETA(id_pedido,referencia,precio,a,b,cc,em,t,w,g,p,i) 
        ...
        $consulta1 = "INSERT INTO CABECERA_PEDIDO_ETA(empresa,comprador,correo,valor_pedido,cantidad_pares,cantidad_referencias,estado_pedido) 
        ...
        $busca = "SELECT MAX(id_pedido) AS ultimo_pedido FROM CABECERA_PEDIDO_ETA";
        ...
        $busca_faltantes = "SELECT * FROM datos_articulos_nb_ETA WHERE style NOT IN(SELECT DISTINCT referencia FROM pedido WHERE id_pedido = $max_pedido)";
        $res_faltantes = sqlsrv_query($conn,$busca_faltantes);
        while($row=sqlsrv_fetch_array($res_faltantes)) {
            $refe = $row["style"];
            $prec = $row["precio_cadena"];
