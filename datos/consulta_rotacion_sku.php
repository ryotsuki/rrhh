...
$vaciar = "TRUNCATE TABLE ROTACION";
$res_vaciar=sqlsrv_query($conn_rot,$vaciar);	
...
if($almacen != -1){
    ...
    $consulta_almacen2 = "AND CODIGO_ALMACEN IN('C1','C2','C3','C4','C5','C6','C7','B1','B2','B3','B4','B5','B6','B7','B8','BA','BB','N1','N2','N3','E1','A11','OAT','OTC','B9','PQ1')";
    $consulta_almacen3 = "AND CODIGO_ALMACEN NOT IN('C1','C2','C3','C4','C5','C6','C7','B1','B2','B3','B4','B5','B6','B7','B8','BA','BB','N1','N2','N3','E1','A11','OAT','OTC','B9','PQ1')";
}
    ...
    //             S.TEMPORADA,
    //             S.CODFAB,
    //             (SELECT SUM(CANTIDAD) AS UNIDADES FROM [CADILAC_ICG].[dbo].MOVIMIENTOS2 WHERE TIPO_MOVIMIENTO = 'TRASPASO' AND ALMACEN_RECIBE = 'E1' AND ALMACEN_ENVIA = 'A1' AND REFERENCIA COLLATE Modern_Spanish_CI_AS = S.REFERENCIA COLLATE Modern_Spanish_CI_AS AND COLOR COLLATE Modern_Spanish_CI_AS = S.COLOR COLLATE Modern_Spanish_CI_AS $consulta_referencia $consulta_color $consulta_marca AND FECHA_ENVIO > CONVERT(VARCHAR,(SELECT MIN(FECHA_ENVIO) FROM [CADILAC_ICG].[dbo].MOVIMIENTOS2 WHERE TIPO_MOVIMIENTO = 'TRASPASO' AND ALMACEN_RECIBE = 'E1' AND ALMACEN_ENVIA = 'A1' $consulta_referencia $consulta_color $consulta_marca ),103)) AS UNIDADES_ENTRANTES,
    //             (SELECT SUM(UNIDADESTOTAL) AS Expr1 FROM dbo.VIEW_AV AS T1 WHERE 1=1 $consulta_referencia $consulta_color $consulta_marca $consulta_fecha2 AND (REFERENCIA = S.REFERENCIA AND COLOR = S.COLOR) AND FACTURADO = 'T' $consulta_almacen1) AS UNIDADES_VENDIDAS,
    //             (SELECT SUM(STOCK) FROM V_STOCK_CON_DETALLE WHERE REFERENCIA = S.REFERENCIA AND COLOR = S.COLOR $consulta_referencia $consulta_color $consulta_almacen2) AS STOCK_ACTUAL,
    //             CONVERT(VARCHAR,(SELECT MAX(FECHA_ENVIO) FROM [CADILAC_ICG].[dbo].MOVIMIENTOS2 WHERE REFERENCIA = S.REFERENCIA COLLATE Modern_Spanish_CI_AS AND COLOR = S.COLOR COLLATE Modern_Spanish_CI_AS AND  TIPO_MOVIMIENTO = 'TRASPASO' AND ALMACEN_RECIBE = 'E1' AND ALMACEN_ENVIA = 'A1' $consulta_referencia $consulta_color $consulta_marca ),103) AS FECHA_ULT_COMPRA,
    //             CONVERT(VARCHAR,(SELECT MIN(FECHA_ENVIO) FROM [CADILAC_ICG].[dbo].MOVIMIENTOS2 WHERE REFERENCIA = S.REFERENCIA COLLATE Modern_Spanish_CI_AS AND COLOR = S.COLOR COLLATE Modern_Spanish_CI_AS AND  TIPO_MOVIMIENTO = 'TRASPASO' AND ALMACEN_RECIBE = 'E1' AND ALMACEN_ENVIA = 'A1' $consulta_referencia $consulta_color $consulta_marca ),103) AS FECHA_PRI_COMPRA,
    //             (SELECT SUM(CANTIDAD) AS UNIDADES FROM [CADILAC_ICG].[dbo].MOVIMIENTOS2 WHERE TIPO_MOVIMIENTO = 'TRASPASO' AND ALMACEN_RECIBE = 'E1' AND ALMACEN_ENVIA = 'A1' AND REFERENCIA COLLATE Modern_Spanish_CI_AS = S.REFERENCIA COLLATE Modern_Spanish_CI_AS AND COLOR COLLATE Modern_Spanish_CI_AS = S.COLOR COLLATE Modern_Spanish_CI_AS $consulta_referencia $consulta_color $consulta_marca AND FECHA_ENVIO = CONVERT(VARCHAR,(SELECT MIN(FECHA_ENVIO) FROM [CADILAC_ICG].[dbo].MOVIMIENTOS2 WHERE TIPO_MOVIMIENTO = 'TRASPASO' AND ALMACEN_RECIBE = 'E1' AND ALMACEN_ENVIA = 'A1' $consulta_referencia $consulta_color $consulta_marca ),103) ) AS PRIMERA_COMPRA
    //         FROM 
    //             dbo.V_STOCK_CON_DETALLE AS S 
    //         WHERE 
    //             1=1 AND
    //             (S.ARTICULO NOT LIKE '%PARCHE%' AND S.ARTICULO NOT LIKE '%PINE%') AND
    //             S.ALMACEN NOT LIKE '%MAYORISTA%' AND S.ALMACEN NOT LIKE '%CENTRAL%'
    //             $consulta_almacen2
    //             $consulta_referencia
    //             $consulta_color
    //             $consulta_marca
    ...
    //             S.TEMPORADA,
    //             S.CODFAB,
    //             (SELECT SUM(UNIDADESTOTAL) AS Expr1 FROM dbo.VIEW_AC AS T1 WHERE 1=1 AND FECHAALBARAN > CONVERT(VARCHAR,(SELECT MIN(FECHAALBARAN) FROM dbo.VIEW_AC AS T1 WHERE REFERENCIA = S.REFERENCIA AND COLOR = S.COLOR AND (NUMSERIE IN ('B11A','B21A','B31A','B41A','B51A','B61A','N11A','C11A','C21A','C31A','C41A','C51A','C61A') AND SUALBARAN LIKE '%.AVG%') $consulta_referencia $consulta_color $consulta_marca $consulta_almacen1 ),103) $consulta_referencia $consulta_color $consulta_marca $consulta_almacen1 AND (REFERENCIA = S.REFERENCIA AND COLOR = S.COLOR) AND (NUMSERIE IN ('B11A','B21A','B31A','B41A','B51A','B61A','N11A','C11A','C21A','C31A','C41A','C51A','C61A') AND SUALBARAN LIKE '%.AVG%') AND ESDEVOLUCION = 'F') AS UNIDADES_ENTRANTES,
    //             (SELECT SUM(UNIDADESTOTAL) AS Expr1 FROM dbo.VIEW_AV AS T1 WHERE 1=1 $consulta_referencia $consulta_color $consulta_marca $consulta_fecha2 AND (REFERENCIA = S.REFERENCIA AND COLOR = S.COLOR) AND FACTURADO = 'T' $consulta_almacen1) AS UNIDADES_VENDIDAS,
    //             (SELECT SUM(STOCK) FROM V_STOCK_CON_DETALLE WHERE REFERENCIA = S.REFERENCIA AND COLOR = S.COLOR $consulta_referencia $consulta_color $consulta_almacen2) AS STOCK_ACTUAL,
    //             CONVERT(VARCHAR,(SELECT MAX(FECHAALBARAN) FROM dbo.VIEW_AC AS T1 WHERE REFERENCIA = S.REFERENCIA AND COLOR = S.COLOR AND (NUMSERIE IN ('B11A','B21A','B31A','B41A','B51A','B61A','N11A','C11A','C21A','C31A','C41A','C51A','C61A') AND SUALBARAN LIKE '%.AVG%') $consulta_referencia $consulta_color),103) AS FECHA_ULT_COMPRA,
    //             CONVERT(VARCHAR,(SELECT MIN(FECHAALBARAN) FROM dbo.VIEW_AC AS T1 WHERE REFERENCIA = S.REFERENCIA AND COLOR = S.COLOR AND (NUMSERIE IN ('B11A','B21A','B31A','B41A','B51A','B61A','N11A','C11A','C21A','C31A','C41A','C51A','C61A') AND SUALBARAN LIKE '%.AVG%') $consulta_referencia $consulta_color),103) AS FECHA_PRI_COMPRA,
    //             (SELECT SUM(UNIDADESTOTAL) AS EXPR1 FROM dbo.VIEW_AC AS T1 WHERE 1=1 $consulta_referencia $consulta_color $consulta_marca $consulta_almacen1 AND REFERENCIA = S.REFERENCIA AND COLOR = S.COLOR AND ESDEVOLUCION = 'F' AND (NUMSERIE IN ('B11A','B21A','B31A','B41A','B51A','B61A','N11A','C11A','C21A','C31A','C41A','C51A','C61A') AND SUALBARAN LIKE '%.AVG%') AND T1.FECHAALBARAN = CONVERT(VARCHAR,(SELECT MIN(FECHAALBARAN) FROM dbo.VIEW_AC AS T1 WHERE REFERENCIA = S.REFERENCIA AND COLOR = S.COLOR AND (NUMSERIE IN ('B11A','B21A','B31A','B41A','B51A','B61A','N11A','C11A','C21A','C31A','C41A','C51A','C61A') AND SUALBARAN LIKE '%.AVG%') $consulta_referencia $consulta_color $consulta_marca $consulta_almacen1 ),103)) AS PRIMERA_COMPRA
    //         FROM 
    //             dbo.V_STOCK_CON_DETALLE AS S 
    //         WHERE 
    //             1=1 AND
...
>
    ...
    <tr>
        ...
        <!--<th class="sortable-column">AJUSTES</th>-->
    ...
    <?php 
        ...
        while($row=sqlsrv_fetch_array($res)) { 
            ...
            //if($row["STOCK_ACTUAL"] > 0){
                ...
                $ent = $row["CODFAB"];
                $precio = $row["PVP"];
                $pri_comp = $row["FECHA_PRI_COMPRA"];
                $ult_comp = $row["FECHA_ULT_COMPRA"];

                if($inicial == 0 || is_null($inicial)){
                    $stock_ini = 0;
                }
                else{
                    $stock_ini = $inicial;
                ...
                if($row["UNIDADES_ENTRANTES"] == 0 || is_null($row["UNIDADES_ENTRANTES"])){
                    $entrantes = 0;
                }
                else{
                    $entrantes = $row["UNIDADES_ENTRANTES"];
                }

                if($row["UNIDADES_VENDIDAS"] == 0 || is_null($row["UNIDADES_VENDIDAS"])){
                    $salientes = 0;
                }
                else{
                    $salientes = $row["UNIDADES_VENDIDAS"];
                }

                if($row["STOCK_ACTUAL"] == 0 || is_null($row["STOCK_ACTUAL"])){
                    $stock_act = 0;
                }
                else{
                    $stock_act = $row["STOCK_ACTUAL"];
                }

                if($row["STOCK_OTROS"] == 0 || is_null($row["STOCK_OTROS"])){
                    $stock_ot = 0;
                }
                else{
                    $stock_ot = $row["STOCK_OTROS"];
                }

                $rotacion_sr = (float)str_replace(",",".",str_replace("%","",$rotacion));
                $rotacion_cr = (float)str_replace(",",".",str_replace("%","",$rotacion3));
    ...
    <tr>
        ...
        <!--<td><?php //echo $ceroa.number_format($row["AJUSTES"],0,',','')?></td>-->
    ...
    <?php
        ...
        $insertar = "INSERT INTO ROTACION(
            ...
            //echo $insertar;
            $res_insertar=sqlsrv_query($conn_rot,$insertar);
        }//}
    ?>
    </tbody>
    <tfoot>
