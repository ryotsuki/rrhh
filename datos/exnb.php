...
$sql = "SELECT 
                ...
                CONVERT(VARCHAR,(SELECT MAX(FECHAALBARAN) FROM dbo.VIEW_AC AS T1 WHERE REFERENCIA = S.REFERENCIA AND COLOR = S.COLOR AND (NUMSERIE IN ('B11A','B21A','B31A','B41A','B51A','B61A','N11A') AND SUALBARAN LIKE '%.AVG%') $consulta_referencia $consulta_color),103) AS FECHA_ULT_COMPRA,
                CONVERT(VARCHAR,(SELECT MIN(FECHAALBARAN) FROM dbo.VIEW_AC AS T1 WHERE REFERENCIA = S.REFERENCIA AND COLOR = S.COLOR AND (NUMSERIE IN ('B11A','B21A','B31A','B41A','B51A','B61A','N11A') AND SUALBARAN LIKE '%.AVG%') $consulta_referencia $consulta_color),103) AS FECHA_PRI_COMPRA,
                (SELECT SUM(UNIDADESTOTAL) AS EXPR1 FROM dbo.VIEW_AC AS T1 WHERE 1=1 $consulta_referencia $consulta_color $consulta_marca $consulta_almacen1 AND REFERENCIA = S.REFERENCIA AND COLOR = S.COLOR AND ESDEVOLUCION = 'F' AND (NUMSERIE IN ('B11A','B21A','B31A','B41A','B51A','B61A','N11A') AND SUALBARAN LIKE '%.AVG%') AND T1.FECHAALBARAN = CONVERT(VARCHAR,(SELECT MIN(FECHAALBARAN) FROM dbo.VIEW_AC AS T1 WHERE REFERENCIA = S.REFERENCIA AND COLOR = S.COLOR AND (NUMSERIE IN ('B11A','B21A','B31A','B41A','B51A','B61A','N11A') AND SUALBARAN LIKE '%.AVG%') $consulta_referencia $consulta_color $consulta_marca $consulta_almacen1 ),103)) AS PRIMERA_COMPRA
            FROM 
