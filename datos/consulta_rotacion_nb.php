...
// }
if($almacen == 1){
    $consulta_almacen1 = "AND CODALMACEN IN('B1','B2','B3','B4','B5','B6','B7','B8','BA','BB','E1','OAT','OTC','B9')";
    $consulta_almacen2 = "AND CODIGO_ALMACEN IN('B1','B2','B3','B4','B5','B6','B7','B8','BA','BB','E1','OAT','OTC','B9')";
}
if($almacen == 2){
    $consulta_almacen1 = "AND CODALMACEN IN ('N1','N2','N3')";
    $consulta_almacen2 = "AND CODIGO_ALMACEN IN ('N1','N2','N3')";
}
if($almacen == -1){
    $consulta_almacen1 = "AND CODALMACEN IN('B1','B2','B3','B4','B5','B6','B7','B8','BA','BB','E1','N1','N2','N3','OAT','OTC','B9')";
    $consulta_almacen2 = "AND CODIGO_ALMACEN IN('B1','B2','B3','B4','B5','B6','B7','B8','BA','BB','E1','N1','N2','N3','OAT','OTC','B9')";
}
 ...
 //echo $consulta_almacen1;  
    $sql = "SELECT 
                ...
                CONVERT(VARCHAR,(SELECT MAX(FECHAALBARAN) FROM dbo.VIEW_AC AS T1 WHERE REFERENCIA = S.REFERENCIA AND COLOR = S.COLOR AND (NUMSERIE IN ('B11A','B21A','B31A','B41A','B51A','B61A','B71A','B81A','BA1A','BB1A','N11A','N21A','N31A') AND SUALBARAN LIKE '%.AVG%') $consulta_referencia $consulta_color),103) AS FECHA_ULT_COMPRA,
                CONVERT(VARCHAR,(SELECT MIN(FECHAALBARAN) FROM dbo.VIEW_AC AS T1 WHERE REFERENCIA = S.REFERENCIA AND COLOR = S.COLOR AND (NUMSERIE IN ('B11A','B21A','B31A','B41A','B51A','B61A','B71A','B81A','BA1A','BB1A','N11A','N21A','N31A') AND SUALBARAN LIKE '%.AVG%') $consulta_referencia $consulta_color),103) AS FECHA_PRI_COMPRA,
                (SELECT SUM(UNIDADESTOTAL) AS EXPR1 FROM dbo.VIEW_AC AS T1 WHERE 1=1 $consulta_referencia $consulta_color $consulta_marca $consulta_almacen1 AND REFERENCIA = S.REFERENCIA AND COLOR = S.COLOR AND ESDEVOLUCION = 'F' AND (NUMSERIE IN ('B11A','B21A','B31A','B41A','B51A','B61A','B71A','B81A','BA1A','BB1A','N11A','N21A','N31A') AND SUALBARAN LIKE '%.AVG%') AND T1.FECHAALBARAN = CONVERT(VARCHAR,(SELECT MIN(FECHAALBARAN) FROM dbo.VIEW_AC AS T1 WHERE REFERENCIA = S.REFERENCIA AND COLOR = S.COLOR AND (NUMSERIE IN ('B11A','B21A','B31A','B41A','B51A','B61A','B71A','B81A','BA1A','BB1A','N11A','N21A','N31A') AND SUALBARAN LIKE '%.AVG%') $consulta_referencia $consulta_color $consulta_marca $consulta_almacen1 ),103)) AS PRIMERA_COMPRA
                --(SELECT SUM(UNIDADESTOTAL) AS Expr1 FROM dbo.VIEW_AC AS T1 WHERE 1=1 AND FECHAALBARAN > CONVERT(VARCHAR,(SELECT MIN(FECHAALBARAN) FROM dbo.VIEW_AC AS T1 WHERE REFERENCIA = S.REFERENCIA AND COLOR = S.COLOR AND (NUMSERIE IN ('B11A','B21A','B31A','B41A','B51A','B61A','B71A','B81A','BA1A','BB1A','N11A','N21A','N31A') AND SUALBARAN LIKE '%BAL%') $consulta_referencia $consulta_color $consulta_marca $consulta_almacen1 ),103) $consulta_referencia $consulta_color $consulta_marca $consulta_almacen1 AND (REFERENCIA = S.REFERENCIA AND COLOR = S.COLOR) AND (NUMSERIE IN ('B11A','B21A','B31A','B41A','B51A','B61A','B71A','B81A','BA1A','BB1A','N11A','N21A','N31A') AND SUALBARAN LIKE '%BAL%')) AS BALANCEOS
                --(SELECT SUM(CANTIDAD) AS UNIDADES FROM [CADILAC_ICG].[dbo].MOVIMIENTOS2 WHERE TIPO_MOVIMIENTO = 'TRASPASO' AND ALMACEN_RECIBE IN('B1','B2','B3','B4','B5','B6','B7','B8','BA','BB','E1','N1','N2','N3') AND ALMACEN_ENVIA IN('B1','B2','B3','B4','B5','B6','B7','B8','BA','BB','E1','N1','N2','N3') AND REFERENCIA COLLATE Modern_Spanish_CI_AS = S.REFERENCIA COLLATE Modern_Spanish_CI_AS AND COLOR COLLATE Modern_Spanish_CI_AS = S.COLOR COLLATE Modern_Spanish_CI_AS $consulta_referencia $consulta_color $consulta_marca AND FECHA_ENVIO > CONVERT(VARCHAR,(SELECT MIN(FECHA_ENVIO) FROM [CADILAC_ICG].[dbo].MOVIMIENTOS2 WHERE TIPO_MOVIMIENTO = 'TRASPASO' AND ALMACEN_RECIBE IN('B1','B2','B3','B4','B5','B6','B7','B8','BA','BB','E1','N1','N2','N3') AND ALMACEN_ENVIA IN('B1','B2','B3','B4','B5','B6','B7','B8','BA','BB','E1','N1','N2','N3') $consulta_referencia $consulta_color $consulta_marca ),103)) AS TRASPASOS                
            FROM 
...
>
    ...
    <tr>
        ...
        <th class="sortable-column">FAMILIA</th>
        <th class="sortable-column">SUBFAM</th>
        <th class="sortable-column">SILUETA</th>
        <th class="sortable-column">MAESTRO</th>
    ...
    ?>
    <tr>
        <td><?php echo str_replace(' ','',($row["REFERENCIA"].$row["COLOR"]))?></td>
        <td><?php echo $row["REFERENCIA"]?></td>
        <td><?php echo $row["ARTICULO"]?></td>
        <td><?php echo $row["FAMILIA"]?></td>
        <td><?php echo $row["SUBFAMILIA"]?></td>
        <td><?php echo $row["SILUETA"]?></td>
        <td><?php echo $row["CODIGO_MAESTRO"]?></td>
